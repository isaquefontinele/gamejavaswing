...
public class Fase {
	...
	/*
	 * Reads the raw string containing all the information about the fase, and
	 * breaks it into different pieces to be used by other functions.
	 */
	private void rawBreak() {
		...
		monsterConfig = tempo;
		itemsTotal = Integer.parseInt(temp[(MATRIX_ROWS + monstersTotal) + 2]
				.trim());
		tempo = "";
		...
		torchesTotal = Integer.parseInt(temp[(MATRIX_ROWS + monstersTotal) + 3
				+ itemsTotal].trim());
		tempo = "";
		for (int i = (MATRIX_ROWS + monstersTotal) + 4 + itemsTotal; i < ((MATRIX_ROWS + monstersTotal)
				+ 4 + itemsTotal + torchesTotal); i++) {
			tempo += temp[i];
	...
	/*
	 * Reads the string containing the matrix and puts the information into an
	 * array
	 */
	...
	/*
	 * Reads the string containing the monsters configurations and puts the
	...
	/*
	 * Reads the string containing the torches configurations and puts the
	 * information into an array
	 */
	...
	/**
	 * Function responsable for getting all the information read and loaded and
		...
		for (int i = 0; i < matrix.length; i++) {
			...
			for (int j = 0; j < matrixRow.length; j++) {
				...
				case "M":
					...
					} else if (matrixMonstersConfigs[monsterIndex][0]
							.equals("B")) {
					...
					// Set max Range
					tempMonster.setMaxRange(Integer
							.parseInt(matrixMonstersConfigs[monsterIndex][2]));
					// Set location
